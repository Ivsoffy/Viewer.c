...
void glView::SetShift_XYZ(QDoubleSpinBox* X, QDoubleSpinBox* Y,
                          QDoubleSpinBox* Z, QDoubleSpinBox* Size) {
  this->Shift_X = X;
  this->Shift_Y = Y;
  this->Shift_Z = Z;
...
void glView::SetRotate_XYZ(QDoubleSpinBox* X, QDoubleSpinBox* Y,
                           QDoubleSpinBox* Z) {
  this->Rotate_X = X;
  this->Rotate_Y = Y;
  this->Rotate_Z = Z;
...
void glView::setColor(QString color) {
  for (ColorPallette pallette : this->colorBackground) {
    if (pallette.Name() == color) *currentBackground = pallette;
  }
}

void glView::AddColor(QColor color, QString Colorname) {
  this->colorBackground.push_back(ColorPallette(
      Colorname, color.redF(), color.blueF(), color.greenF(), color.alphaF()));
...
int glView::GetVectors() { return this->count_vector; }
int glView::GetSurface() { return this->count_surface; }
...
void glView::mouseMoveEvent(QMouseEvent* event) {
  if (!shift) {
    ...
    this->diff.x_move += (event->scenePosition().x() - this->position_x) /
                         speed * (this->diff.size * 0.5);
    this->diff.y_move += (event->scenePosition().y() - this->position_y) /
                         speed * (this->diff.size * 0.5);
    this->position_x = event->scenePosition().x();
    this->position_y = event->scenePosition().y();
    this->Shift_X->setValue(this->diff.x_move);
...
void glView::keyPressEvent(QKeyEvent* event) {
  if (event->key() == Qt::Key_Shift) this->shift = 1;
}

void glView::keyReleaseEvent(QKeyEvent* event) {
  if (event->key() == Qt::Key_Shift) this->shift = 0;
}
...
void glView::TakePic() {
  ...
  e = e.scaled(
      640, 480,
      Qt::IgnoreAspectRatio);  // aspectratio is goddamanit trash hate this this
                               // crush gif that bunch of shit
  frames.push_back(e);
  if (frames.size() == 30) {
    timer->stop();
    SaveGif();
...
void glView::SaveGif() {
  ...
  GifWriter writer;
  GifBegin(&writer, fileGIF.toStdString().c_str(), width, height, delay, 8,
           true);
  for (int i = 0; i < 30; i++) {
...
void glView::paintGL() {
  if (this->count_vector) {
    ...
    glClearColor(
        this->currentBackground->Red(), this->currentBackground->Green(),
        this->currentBackground->Blue(), this->currentBackground->Alpha());
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    // команды отрисовки векторов
    glColor3f(this->colorVertex->Red(), this->colorVertex->Green(),
              this->colorVertex->Blue());
    ...
    for (int i = 0; i < this->count_surface; i++) {
      ...
      for (int j = 0; j < surface[i].number_dot_surface; j++) {
        ...
        glVertex3d(this->current_vectors[index].x,
                   this->current_vectors[index].y,
                   this->current_vectors[index].z);
      }
      glEnd();
    }
    ...
    if (this->pointType != 2) {
      glColor3f(this->colorEdges->Red(), this->colorEdges->Green(),
                this->colorEdges->Blue());
      glPointSize(this->size_edges);

      glBegin(GL_POINTS);
      for (int i = 0; i < this->count_vector; i++) {
        glVertex3d(this->current_vectors[i].x, this->current_vectors[i].y,
                   this->current_vectors[i].z);
      }
      glEnd();
    }
  }
